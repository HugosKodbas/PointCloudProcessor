...
def is_parallel(model1, model2, angle_threshold=3.0):
...
def ransac_line_fitting(x, y, max_iterations, threshold, min_inliers, angle_threshold, wall_post_processing=False, door_post_processing=False):
    ...
    while len(available_points) >= min_inliers:
        ...
        while iterations_per_model > 0:
            ...
            sample_indices = SEED.sample(range(len(available_points_list)), 2)
    ...
    if door_post_processing:
        ...
        models, parallel_pairs, parallel_groups = detect_parallel_lines(models, 0.15, 3.0) # angle raised 0.5→3.0 deg; dist tightened 0.40→0.15 m
    ...
    if wall_post_processing:
        ...
        models = bridge_collinear_walls(models, separation_threshold=0.50, lateral_threshold=0.2, angle_threshold=3.0) # Default: 0.5, 0.20, 0.5)
...
def process_room(wall_csv: str, door_csv: str, window_csv: str, out_json: str, show_plot: bool = False, out_plot: str = None):
    ...
    # ransac_line_fitting) so near-duplicate scan faces are merged, while keeping
    # the distance threshold tight (0.15 m) to preserve genuine two-surface pairs.
    walls, _, dup_groups = detect_parallel_lines(walls, distance_threshold=0.15, angle_threshold=3.0)
    walls = remove_parallel_walls(walls, dup_groups)
    walls = relabel_walls(walls)

    # ── extend short walls using window evidence ───────────────────────────────
    # Some walls (e.g. a glass face behind a structural column) are only partially
    # detected because the scanner returns sparse pts in the column zone. The window
    # RANSAC segment correctly spans the full glass height — use it to extend the
    # matched wall before snapping so the window opening is cut at full height.
    windows_for_ext = filter_window_segments(
                        windows_raw,
                        walls,
    ...
    def _extend_walls_using_windows(walls, windows, max_extension_factor=2.0):
        ...
        for win in windows:
            ...
            wall = next((w for w in walls if w['wall_id'] == wid), None)
            if wall is None: continue
            wp1  = _to_xy(wall['endpoints'][0])
            wp2  = _to_xy(wall['endpoints'][1])
            wdir = _seg_dir(wp1, wp2)
            wlen = line_length(wp1, wp2)
            vp1  = _to_xy(win['endpoints'][0])
            vp2  = _to_xy(win['endpoints'][1])
            t1, t2     = float(np.dot(vp1-wp1, wdir)), float(np.dot(vp2-wp1, wdir))
    ...
    walls = _extend_walls_using_windows(walls, windows_for_ext)

    # Keep only the door which lies closest to a wall.
    doors = filter_door_segments_keep_frame_opening(
                        doors,
                        walls,
                        threshold=0.35,           # raised: doors sit ~0.30 m from wall midpoints
                        use_angle=True,
                        angle_threshold_deg=20.0,
                        angle_weight=0.10,
                        max_endpoint_wall_distance=None  # disabled: endpoints extend beyond wall bounds
                    )
    ...
    windows = filter_window_segments(
                        windows_raw,
                        walls,
                        max_midpoint_wall_distance=0.20,  # tightened from 0.30
                        angle_threshold_deg=10.0,          # actual diffs are 0.7-4.5 deg
                        min_length=0.20,
                        )
